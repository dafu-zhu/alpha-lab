...
class TestFieldRegistry:
    def test_all_fields_populated(self):
        from alphalab.data.features.registry import ALL_FIELDS
    ...
    def test_valid_field_names_matches(self):
        from alphalab.data.features.registry import ALL_FIELDS, VALID_FIELD_NAMES
    ...
    def test_close_is_price_volume(self):
        from alphalab.data.features.registry import ALL_FIELDS, FieldCategory, FieldSource
    ...
    def test_debt_is_derived(self):
        from alphalab.data.features.registry import ALL_FIELDS, FieldCategory
...
class TestBuildOrder:
    def test_build_order_returns_list(self):
        from alphalab.data.features.registry import get_build_order
        order = get_build_order()
        assert isinstance(order, list)
        assert len(order) > 0

    def test_build_order_contains_all_fields(self):
        from alphalab.data.features.registry import get_build_order, ALL_FIELDS
        order = get_build_order()
