...
from alphalab.data.master.security_master import SecurityMaster, SymbolNormalizer
...
class TestSymbolNormalizer:
    ...
    @patch('alphalab.data.master.security_master.fetch_all_stocks')
    @patch('alphalab.data.master.security_master.setup_logger')
...
class TestSecurityMasterInitialization:
    ...
    @patch('alphalab.data.master.security_master.SecurityMaster._load_from_local')
    @patch('alphalab.data.master.security_master.setup_logger')
...
class TestSecurityMasterSecOperations:
    ...
    def test_fetch_sec_exchange_mapping(self):
        ...
        with patch('alphalab.data.master.security_master.requests.get', return_value=mock_response):
...
class TestGICSMapping:
    ...
    def test_load_gics_mapping_missing_file(self):
        ...
        with patch('alphalab.data.master.security_master.GICS_MAPPING_PATH') as mock_path:
...
class TestYfinanceMetadata:
    ...
    def test_fetch_yfinance_metadata_success(self):
        ...
        with patch.dict('sys.modules', {'yfinance': Mock()}):
            ...
            with patch('alphalab.data.master.security_master.time.sleep'):
...
class TestOpenFIGIIntegration:
    ...
    def test_fetch_openfigi_mapping_success(self):
        ...
        with patch('alphalab.data.master.security_master.requests.post', return_value=mock_response):
            with patch('alphalab.data.master.security_master.RateLimiter') as mock_rl:
    ...
    def test_fetch_openfigi_mapping_api_error(self):
        ...
        with patch('alphalab.data.master.security_master.requests.post') as mock_post:
    ...
    def test_fetch_openfigi_mapping_batching(self):
        ...
        with patch.dict(os.environ, {'OPENFIGI_API_KEY': 'test-key'}):
            with patch('alphalab.data.master.security_master.requests.post', side_effect=mock_post):
                with patch('alphalab.data.master.security_master.RateLimiter') as mock_rl:
                    mock_rl.return_value.acquire = Mock()
    ...
    def test_fetch_openfigi_uses_api_key_when_available(self):
        ...
        with patch.dict(os.environ, {'OPENFIGI_API_KEY': 'test-key'}):
            with patch('alphalab.data.master.security_master.requests.post', return_value=mock_response) as mock_post:
                with patch('alphalab.data.master.security_master.RateLimiter') as mock_rl:
                    mock_rl.return_value.acquire = Mock()
...
class TestOpenFIGIRetryBehavior:
    ...
    def test_fetch_openfigi_429_retries_with_backoff(self):
        ...
        with patch('alphalab.data.master.security_master.requests.post', side_effect=mock_post):
            with patch('alphalab.data.master.security_master.RateLimiter') as mock_rl:
                mock_rl.return_value.acquire = Mock()
                with patch('alphalab.data.master.security_master.time.sleep') as mock_sleep:
                    result = sm._fetch_openfigi_mapping(['AAPL'])
