"""Unified CLI for US Equity Data Lake.

Commands:
    qdl --master               Build security_master + calendar_master
    qdl --all                  Master build + download all data
    qdl --ticks                Download daily ticks only
    qdl --fundamental          Download fundamentals only
    qdl --top-3000             Download top 3000 universe only
    qdl --features             Build feature wide tables only
"""
...
def _build_master(logger) -> None:
    ...
    from quantdl.utils.logger import console_log
...
def main():
    parser = argparse.ArgumentParser(
        description="US Equity Data Lake CLI",
